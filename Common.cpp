...
namespace Utility
{
	...
	void UpdateContextMenu( ContextMenuStrip^ strip, Decoration^ decoration )
	{
		if( StringTable::japanese )
		{
			if( decoration->eng_name )
				AddMenuItem( strip->Items, decoration->eng_name, true, DecorationID, decoration->ping_index );
		}
		else AddMenuItem( strip->Items, decoration->name, true, DecorationID, decoration->ping_index );

		AddMenuItem( strip->Items, decoration->jap_name, true, DecorationID, decoration->ping_index );
		strip->Items->Add( L"-" );
	...
	void UpdateContextMenu( ContextMenuStrip^ strip, Armor^ armor )
	{
		...
		{
			if( armor->eng_name )
				AddMenuItem( strip->Items, armor->eng_name, true, ArmorID, armor->ping_index );
		}
		else AddMenuItem( strip->Items, armor->name, true, ArmorID, armor->ping_index );

		AddMenuItem( strip->Items, armor->jap_name, true, ArmorID, armor->ping_index );
		
		strip->Items->Add( L"-" );
		strip->Items->Add( SlotString( armor->num_slots ) );
		strip->Items->Add( L"-" );
		strip->Items->Add( ColonString( Rarity ) + ( armor->rarity == 8 ? L"X" : Convert::ToString( armor->rarity ) ) );
